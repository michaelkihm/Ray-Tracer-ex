...
glm::vec3 getMirrorDirection(glm::vec3 rayDirection, glm::vec3 normal);
  ...
  for(int i = 0; i < image.rows; i++)
  {
    for(int j=0; j < image.cols; j++)
    {
      ...
      if(hit){
        ...
        image.at<Vec3b>(i,j) = Vec3b(static_cast<uchar>(color.b*255.0), static_cast<uchar>(color.g*255.0),static_cast<uchar>(color.r*255.0));
      }
...
glm::vec3 ComputeLight(tuple<float,shared_ptr<Primitive>,vec3> hit, vec3 eye_pos, int depth){
  ...
  if(depth){
    for(auto c_light : lights)
    {
      ...
      if( c_light.getType() == LightType::point )
      {
        ...
        attenuation = 1.0/(light_attenuation[0]+light_attenuation[1]*distance_obj_light + light_attenuation[2]*pow(distance_obj_light,2));
      }
      else
      {
    ...
    vec3 new_direction = getMirrorDirection(get<2>(hit) ,normal);
  ...
  {
    if(out_color[i] > 1) out_color[i]=1.0;
    else if(out_color[i] < 0) out_color[i]=0.0;
  }
